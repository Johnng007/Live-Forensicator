...
# Get the latest tag name from the latest release
$latestTagName = $latestRelease.tag_name

# Get the latest commit hash from the latest tag
$latestCommitHash = (Invoke-RestMethod -Uri "https://api.github.com/repos/$repoOwner/$repoName/git/ref/tags/$latestTagName").object.sha

# Get the local commit hash
$localCommitHash = git rev-parse HEAD

# Compare the latest commit hash with the local commit hash
if ($localCommitHash -ne $latestCommitHash) {
    #Write-Output "There is an update available on GitHub."
    Write-Host -Fore Cyan "[!] An update is available on Github"
} else {
   # Write-Output "There are no updates available on GitHub."
...
$BlackWidowLogo >> $FinalDes
$IndexNavStyle >> $FinalDes
...
<ul id=''list-nav''>
  <li><a href="#" onclick=''loadContent("home.html")''>Home</a></li>
  <li><a href="#" onclick=''loadContent("users.html")''>Users `& Accounts</a></li>
  <li><a href="#" onclick=''loadContent("system.html")''>System Information</a></li>
  <li><a href="#" onclick=''loadContent("network.html")''>Network Information</a></li>
  <li><a href="#" onclick=''loadContent("processes.html")''>System Processes</a></li>
  <li><a href="#" onclick=''loadContent("others.html")''>Other Information</a></li>
  <li><ul>Event Log Analysis</ul></li>
  <li><a href="#" onclick=''loadContent("evtx_user.html")''>User Actions</a></li>
  <li><a href="#" onclick=''loadContent("evtx_logons.html")''>Logon Events</a></li>
  <li><a href="#" onclick=''loadContent("evtx_object.html")''>Object Access</a></li>
  <li><a href="#" onclick=''loadContent("evtx_process.html")''>Process Execution</a></li>
  <li><a href="#" onclick=''loadContent("evtx_suspicious.html")''>Suspicious Activities</a></li>
  
</ul>
</div>
'

$IndexNavMenu >> $FinalDes


$IndexDiv = '<div style=''width: 80%; margin: 0 auto;padding-bottom: 56.25%;padding-top: 25px;height: 0;''>'
$IndexIframe = '<iframe id=''content'' src=''home.html'' style='' width: 100%; height: 100%; position: absolute;padding-left:20px'' frameBorder=''0''></iframe> '

$IndexDiv >> $FinalDes
$IndexIframe >> $FinalDes
'</div>' >> $FinalDes

'<center>' >> $FinalDes
'<div>'  >> $FinalDes
echo "<h3> Evidence gathered from  $env:computername  by  $operator at: $Endtimecheck with: <a href='https://github.com/Johnng007/Live-Forensicator' >Live Forensicator</a> </h3>" | Out-File -Append $FinalDes
'</div>'  >> $FinalDes
'</center>' >> $FinalDes
...
###########################################################################################################
#region ########################## CREATING AND FORMATTING THE HOME FILE  ################################
###########################################################################################################
...
$BrowserButton = '<h3>BROWSING HISTORY:</h3><table></table><a href=''BROWSING_HISTORY''>View Browsing History</a>'

$BrowserButton >> $HomeDes
...
if ((gwmi win32_computersystem).partofdomain -eq $true) {
    ...
    echo "<h3>Group Policy Report</h3><table><a href='GPOReport.html' target='_blank'>View the GPO report </a></table><br>"  | Out-File -Append $HomeDes
    Write-Host -Fore Cyan "[!] Done"
} else {
...
& $PSScriptRoot\Forensicator-Share\winpmem_mini_x64_rc2.exe RAM\$env:computername.raw | Out-Null
   ...
   echo "<h3>WINPMEM RAM CAPTURE:</h3><table></table><a href='RAM'>View RAM Capture</a><br>"     | Out-File -Append $HomeDes
...
& $PSScriptRoot\Forensicator-Share\BrowsingHistoryView64.exe /sverhtml "BrowserHistory.html" /SaveDirect /HistorySource 1 /VisitTimeFilterType 1 /LoadIE 1 /LoadFirefox 1 /LoadChrome 1 /LoadSafari 1
   echo "<h3>BROWSING HISTORY:</h3><table></table><a href='BrowserHistory.html'>View Browsing History</a><br>"     | Out-File -Append $HomeDes
...
& $PSScriptRoot\Forensicator-Share\etl2pcapng64.exe PCAP\$env:computername.et1 PCAP\$env:computername.pcap
   echo "<h3>NETWORK TRACE:</h3><table></table><a href='PCAP'>View PCAP FILES</a><br>"     | Out-File -Append $HomeDes
...
Foreach($log in $logArray){
    ...
    echo "<h3>EVENT LOGS:</h3><table></table><a href='EVTLOGS'>View Event Logs</a><br>"      | Out-File -Append $HomeDes
...
if (!(gci C:\inetpub\logs\ *.log)){
   ...
   echo "<h3>IIS Logs</h3><table></table><a href='IISLogs' >View IIS Logs</a><br>"           | Out-File -Append $HomeDes
...
If ($FoundRegKey)
	...
	echo "<h3>TomCat Logs</h3><table></table><a href='TomCatLogs'>View TomCat Logs</a><br>"   | Out-File -Append $HomeDes
...
if ($LOG4J) {
   ...
   $DriveList = (Get-PSDrive -PSProvider FileSystem).Root
      ...
      echo "<h3>Discovered Log4j</h3><table></table><a href='LOG4J' >View File</a><br>"                                 | Out-File -Append $HomeDes
...
$html = $logonDetails | ConvertTo-Html -As Table -Fragment




'<details>' >> $LogonEvtx
echo "<summary>Successful Logon Events</summary>"                                                                | Out-File -Append $LogonEvtx
# Output the HTML to a file
$html | Out-File -Append $LogonEvtx
'</details>' >> $LogonEvtx
'<br><br>' >> $LogonEvtx
...
'<details>' >> $LogonEvtx
echo "<summary>Failed Logon Events</summary>"                                                                | Out-File -Append $LogonEvtx
# Output the HTML to a file
$html | Out-File -Append $LogonEvtx 
'</details>' >> $LogonEvtx
'<br><br>' >> $LogonEvtx
...
$HtmlTable = "<table><tr><th>Time</th><th>User</th><th>Object</th><th>Access Type</th></tr>"
...
#$HtmlTable | Out-File -FilePath "ObjectAccessEvents.html"


'<details>' >> $ObjectEvtx
echo "<summary>Object Access Events - Last 1000 days</summary>"                                                                | Out-File -Append $ObjectEvtx
# Output the HTML to a file
$HtmlTable | Out-File -Append $ObjectEvtx
'</details>' >> $ObjectEvtx
'<br><br>' >> $ObjectEvtx
...
$tableHeader = "<table><tr><th>Time</th><th>Process Name</th><th>Process ID</th><th>User</th></tr>"
...
#$html | Out-File -FilePath $outputFilePath

'<details>' >> $ProcessEvtx
echo "<summary>Process Execution - Last 1000 days</summary>"                                                                | Out-File -Append $ProcessEvtx
# Output the HTML to a file
$html | Out-File -Append $ProcessEvtx
'</details>' >> $ProcessEvtx
'<br><br>' >> $ProcessEvtx
...
$SuspiciousEvtxStyle > $SuspiciousEvtx

# Define the event log and severity level
$logName = "Security"
$severityLevels = @{ 16 = "Critical"; 8 = "Error"; 4 = "Warning"; 2 = "Information" }
...
#$exec = [Regex]::Escape($searchRegex)
#$PS = [Regex]::Escape($searchRegex1)

#$searchRegex2 = ($all_suspicious_chunk3 -join "|")

# Filter the malicious events and create a new object with severity, timestamp, date, event ID, and description properties
$maliciousEvents = $entries | Where-Object { $_.Message -match $searchRegex, $searchRegex1 } | ForEach-Object {
    [PSCustomObject] @{
        Severity = $severityLevels[$_.EntryType]
        Timestamp = $_.TimeGenerated
        Date = $_.TimeGenerated.ToShortDateString()
        EventID = $_.EventID
        Description = $_.Message
    }
}

# Create an HTML table with severity, timestamp, date, event ID, and description columns
$html = $maliciousEvents | ConvertTo-Html -Fragment -Property Severity, Timestamp, Date, EventID, Description |
        ForEach-Object { $_ -replace '<table>', '<table border="1" cellpadding="2" cellspacing="0">' }

# Output the HTML table
#Write-Output $html

#$outputFilePath = "malicious.html"

#$html | Out-File -FilePath $outputFilePath


'<details>' >> $SuspiciousEvtx
echo "<summary>Suspicious Activities</summary>"                                                                | Out-File -Append $SuspiciousEvtx
# Output the HTML to a file
$html | Out-File -Append $SuspiciousEvtx 
'</details>' >> $SuspiciousEvtx
'<br><br>' >> $SuspiciousEvtx
...
echo "<h3><u>USERS & GROUPS ACTIVITIES</u></h3> "                                                                           | Out-File -Append $EVTXDes
