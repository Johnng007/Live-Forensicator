...
param( 
    ...
    #[Parameter(ParameterSetName='log4j')]
    [String]$log4j,
...
Write-Host ''
Write-Host -Fore DarkCyan   'Live Forensicator v 1.0'
Write-Host ''
Write-Host -Fore DarkCyan   'Examines the host for suspicious activities and grabs required data for further forensics.'
Write-Host -Fore DarkCyan   'This script is part of the Black Widow Tools.'
...
# Moving in to the Artifact folder
#Set-Location Artifacts
#$Loc = Get-Location
# creating a directory inside Artifact folder to store the artifacts of this host
mkdir $env:computername -Force

# Moving again into the new folder
Set-Location $env:computername

# Getting the current working directory as variable
#$FirstLoc = Get-Location
#$SecLoc = $env:computername
#$CompName = $env:computername
#$GetLoc = '$FirstLoc\$SecLoc'
...
Write-Host -Fore Cyan '        Gatthering Network & Network Settings '
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan     '        Gatthering Users and Account Info '
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
##################################################
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan     '        Gatthering Installed Programs '
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan '        Gatthering System Information '
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan '        Gatthering Processes and Tasks '
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
#$Services2 = Get-CimInstance –ClassName Win32_Service | Select-Object Name, DisplayName, StartMode, State, PathName, StartName, ServiceType | ConvertTo-Html -Fragment 
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan     '        Checking Registry for persistance '
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan '        Running Other Final Checks '
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan '        Lets Get hold of some weblogs '
Write-Host -Fore DarkCyan '------------------------------------------------------'

#create IIS log Dirs
mkdir IISLogs

$IISLogs = Copy-Item -Path 'C:\inetpub\logs\*' -Destination 'IISLogs' -Recurse

#checking for Tomcat and trying to get log files
mkdir TomCatLogs
$FoundRegKey = $null
...
If ($FoundRegKey)
	...
	$TomcatLogs = Copy-Item -Path '$logfolder\*' -Destination '$GetLoc\TomCatLogs' -Recurse
    }
Else
    {
...
<ul id='list-nav'>
  ...
  <li><a href='users.html'>User & Account Information</a></li>
  <li><a href='system.html'>System Information</a></li>
  <li><a href='network.html'>Network Information</a></li>
  <li><a href='processes.html'>Running & Startup Processes</a></li>
...
echo "<h3><center> Start Time and Date: $currenttime </center></h3><br>"                                            | Out-File -Append $FinalDes
echo "<h3><center> End Time and Date: $endtimecheck </h3><br>"                                                      | Out-File -Append $FinalDes
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan '        COLLECTING GPO RESULTS'
Write-Host -Fore DarkCyan '------------------------------------------------------'

$GP = GPRESULT /H GPOReport.html
...
mkdir RAM
#Set-Location ..
& $PSScriptRoot\winpmem_mini_x64_rc2.exe RAM\$env:computername.raw | Out-Null

Write-Host -Fore DarkCyan 'Done....'
#Set-Location $env:computername  
...
Write-Host -Fore DarkCyan '------------------------------------------------------'
Write-Host -Fore Cyan '        Gettting hold of some event logs'
Write-Host -Fore DarkCyan '------------------------------------------------------'
...
if ($log4j) {
   ...
   Write-Host -Fore DarkCyan '-----------------------------------------------------------------'
   Write-Host -Fore Cyan '        Checking for log4j on all drives .....this may take a while'
   Write-Host -Fore DarkCyan '-----------------------------------------------------------------'
   mkdir LOG4J	
   # Checking for Log4j
   ...
   Write-Host -Fore DarkCyan 'Check Completed'
...
Write-Host -Fore DarkCyan '-----------------------------------------------------------------'
Write-Host -Fore Cyan '        Hang on, the Forensicator is compiling your results'
Write-Host -Fore DarkCyan '-----------------------------------------------------------------'
