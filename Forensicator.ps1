...
if ((gwmi win32_computersystem).partofdomain -eq $true) {
    ...
    Write-Host -Fore DarkCyan "[*] Computer is not on the domain...moving on"
}
...
& $PSScriptRoot\winpmem_mini_x86.exe RAM\$env:computername.raw | Out-Null
...
& $PSScriptRoot\etl2pcapng86.exe PCAP\$env:computername.et1 PCAP\$env:computername.pcap
...
if ($RegRun) {echo "<h3>Persistance in Registry1</h3><table>$RegRun</table><br>"                                | Out-File -Append $ProcDes}
if ($RegRunOnce) {echo "<h3>Persistance in Registry2</h3><table>$RegRunOnce</table><br>"                            | Out-File -Append $ProcDes}
if ($RegRunOnceEx) {echo "<h3>Persistance in Registry3</h3><table>$RegRunOnceEx</table><br>"                          | Out-File -Append $ProcDes}
...
if ($PSHistory) {echo "<h3>500Days Powershell History</h3><table>$PSHistory</table><br>"                              | Out-File -Append $OtherDes}
